...
adf_base = subset(adf, :levee => ByRow(isequal(false)), :slr_scen => ByRow(isequal("medium")))
mdf_base = subset(mdf, :levee => ByRow(isequal(false)), :slr_scen => ByRow(isequal("medium")), :risk_averse => ByRow(isequal(0.3)))

adf_base_high = subset(adf_base, :risk_averse => ByRow(isequal(0.3)))
adf_base_low = subset(adf_base,  :risk_averse => ByRow(isequal(0.7)))


adf_levee = subset(adf, :levee => ByRow(isequal(true)), :slr_scen => ByRow(isequal("medium")))
mdf_levee = subset(mdf, :levee => ByRow(isequal(true)), :slr_scen => ByRow(isequal("medium")), :risk_averse => ByRow(isequal(0.3)))
...
hidespines!(ax2, :t, :r)
"""
ax3 = Axis(ga[1, 1], title = "No Levee", ylabel = "Floodplain Population (%)", xlabel = "Model Timestep (year)", 
limits = ((0,50), nothing))
hidespines!(ax3, :t, :r)
...
for i in eachindex(unique(adf_base_high.seed))
    ...
    color = (palette[1], 0.35), linewidth = 1, transparency = true, label = "High Risk Aversion")
    CairoMakie.lines!(ax3, adf_base_low.step[1+(i-1)*51:i*51], adf_base_low.flood_pop_change[1+(i-1)*51:i*51], 
    color = (palette[2], 0.35), linewidth = 1, transparency = true, label = "Low Risk Aversion")
end
