...
xticks = ([0,1,3,5,7,10], string.([0,1,3,5,7,10])), linecolor = [palette[1] palette[3] palette[2]], 
legend = :outerbottom, legendcolumns = 3, legend_foreground_color = :transparent, dpi = 300)
Plots.xlabel!("Flood Events per Decade")
Plots.ylabel!("Movement Probability")

log_fig = Plots.plot(x.*10, y,  lw = 3, xticks = ([0,1,3,5,7,10], string.([0,1,3,5,7,10])), linecolor =  palette[3], 
legend = false, dpi = 300)
Plots.xlabel!("Flood Events per Decade")
Plots.ylabel!("Movement Probability")

savefig(log_fig, joinpath(pwd(),"figures/log_func_noRA.svg"))

log_scale_fig = Plots.plot(x.*10,[y1 y1_scale y2_scale y3_scale], label = ["fixed effect = 0" " fixed effect = 0.03" "fixed effect = 0.05" "fixed effect = 0.07"], lw = 3,
