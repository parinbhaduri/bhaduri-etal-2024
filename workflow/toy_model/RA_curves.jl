...
#Create function
function move_curve(x; ra = 0.3, scale = 1, base_prob = 0.025)
    if x == 0.0
        prob = base_prob
    else
        init_prob = 1/(1+ exp(-10((scale * x) - ra))) + base_prob
...
#Fixed Effect Curves
y1_scale = move_curve.(x; ra = 0.3, scale = 0.0)
y2_scale = move_curve.(x; ra = 0.3, scale = 0.5)
y3_scale = move_curve.(x; ra = 0.3, scale = 0.7)
...
log_scale_fig = Plots.plot(x.*10,[y1 y1_scale y2_scale y3_scale], label = ["fixed effect = 0" " fixed effect = 0.3" "fixed effecte = 0.5" "fixed effect = 0.7"], lw = 3,
